...
describe("GEAreaTable tests", () => {
  ...
  test("has the expected column headers expand toggles and formatted cells", async () => {
    ...
    expect(
      screen.getByTestId(`${testId}-cell-row-0.0-col-quarter`),
    ).toHaveTextContent("");
    ...
    expect(detailLink).toHaveAttribute("href", "/coursedetails/20244/30247");
    ...
    ).toHaveTextContent("M W F");
    expect(
      screen.getByTestId(`${testId}-cell-row-0-col-time`).textContent,
    ).toMatch(/8:00.*8:50/i);
    expect(
    ...
    const row1CourseId = screen.getByTestId(
      `${testId}-cell-row-1-col-courseId`,
    );
